...
def postprocess(pred, IM=[], conf_thres=0.5, iou_thres=0.2):
    ...
    # 首先将输出转换为[1, 1029, 68]的形状以便处理
    if pred.shape == (1, 68, 1029):
        # 修复转置操作，使用正确的维度顺序
        pred = pred.transpose(0, 2, 1)  # 转换为[1, 1029, 68]
...
if __name__ == "__main__":
    ...
    print(f"前5个像素值示例: {img_pre[0, 0, 100:105, 100:105].flatten()}")
    ...
    try:
        ...
        # 模型推理
        print("\n执行ONNX模型推理...")
        outputs = session.run(None, {input_name: img_pre})
        ...
        if len(result.shape) >= 3 and result.shape[1] >= 2:
            for i in range(min(2, result.shape[1])):
                print(f"检测结果 {i+1}: {result[0, i, :10]}")
        
        # 确保输出形状符合 [1, 68, 1029]或其他ONNX模型输出格式
        print(f"\n模型输出形状: {result.shape}")
