...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"数值范围: [{result_np.min():.6f}, {result_np.max():.6f}]")
        print(f"平均值: {result_np.mean():.6f}")
        
        # 打印部分输出数据示例
        print("\n输出数据示例（前2个检测结果的前10个值）:")
        if result_np.shape[1] >= 2:
            for i in range(min(2, result_np.shape[1])):
                print(f"检测结果 {i+1}: {result_np[0, i, :10]}")
